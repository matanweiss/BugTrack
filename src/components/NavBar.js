...
const NavBar = ({ title, setIsProjectSelected }) => {
  ...
  const handleSelectProjectClick = () => setIsProjectSelected(false);
  ...
  return (
    <nav className={`${scrolledDown ? 'bg-red-700 shadow-lg text-white' : 'bg-transparent text-red-700'} z-10 sticky top-0 h-16 font-body pl-2 pr-3  transition`}>
      <div className="max-w-3xl mx-auto flex items-center justify-between">
        ...
        <button className={`${title ? 'flex' : 'hidden'} items-center space-x-2`} onClick={handleSelectProjectClick}>
          <h3 className='md:text-4xl'>{title}</h3>
          <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
        </button>
